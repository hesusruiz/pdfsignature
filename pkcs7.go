...
func verifySignature(p7 *PKCS7, signer signerInfo) error {
	...
	//Get the signed data from the PKCS structure
	signedData := p7.Content

	//If there are Authenticated Attributes in the signature, the signature was not
	//done over the raw PDF content, but rather over some calculated attributes
	if len(signer.AuthenticatedAttributes) > 0 {

		//Allocate space for the Digest Attribute from the signature
		var digest []byte
		...
		//Get the hash algorithm that was used when signing
		...
		//Compute the hash over the raw PDF data
		h.Write(p7.Content)
		computed := h.Sum(nil)

		//Compare the computed hash with the Digest obtained from the signature
		...
		// TODO(jrm): Verify certificate chain
		// TODO(jrm): Verify signingTime against certificate NotAfter/NotBefore

		//Obtain the Signed Data from the signature AuthenticatedAttributes
	...
	//Obtain the signer certificate from the Certificates structure inside the PKCS7 struct
	cert := getCertFromCertsByIssuerAndSerial(p7.Certificates, signer.IssuerAndSerialNumber)
	if cert == nil {
		return errors.New("pkcs7: No certificate for signer")
	}

	//Get the algorithm that was used in the signature
